...
func (s *ReportService) BuildMonthlyReport(ctx context.Context, year, month int) (domain.MonthlyReport, error) {
	report := domain.MonthlyReport{Year: year, Month: month}

	income, expenses, investments, err := s.reports.MonthlyTotals(ctx, year, month)
	if err != nil {
		return report, err
	}
	...
	report.ByCategory, err = s.reports.CategoryTotals(ctx, year, month)
	if err != nil {
		return report, err
	}

	report.BySubtype, err = s.reports.SubtypeTotals(ctx, year, month)
	if err != nil {
		return report, err
	}

	report.TopExpenses, err = s.reports.TopExpenses(ctx, year, month, 5)
	if err != nil {
		return report, err
	}
...
func (s *ReportService) CalculateKPIs(ctx context.Context, year int) (domain.KPIReport, error) {
	opening, income, expenses, investments, fixed, err := s.reports.YearlyKPIs(ctx, year)
	if err != nil {
...
func (s *ReportService) GetTrend(ctx context.Context, year int) ([]domain.MonthlyReport, error) {
	return s.reports.MonthlyTrend(ctx, year)
}

// GetBudgetVsActual returns budget vs actual spending by category for a given month.
func (s *ReportService) GetBudgetVsActual(ctx context.Context, year, month int) ([]domain.CategorySummary, error) {
	return s.reports.BudgetVsActual(ctx, year, month)
}
...
func (s *ReportService) BuildAnnualReport(ctx context.Context, year int) (domain.AnnualReport, error) {
	report := domain.AnnualReport{Year: year}

	kpis, err := s.CalculateKPIs(ctx, year)
	if err != nil {
		return report, err
	}
	report.KPIs = kpis

	trend, err := s.reports.MonthlyTrend(ctx, year)
	if err != nil {
		return report, err
	}
	report.MonthlyTrend = trend

	cats, err := s.reports.YearlyCategoryTotals(ctx, year)
	if err != nil {
		return report, err
	}
	report.CategoryTotals = cats

	top, err := s.reports.YearlyTopExpenses(ctx, year, 10)
	if err != nil {
		return report, err
	}
...
func (s *ReportService) GetMonthlyHistory(ctx context.Context, year, beforeMonth int) ([]domain.MonthlyReport, error) {
	return s.reports.MonthlyHistory(ctx, year, beforeMonth)
}

// GetYearlyCategoryTotals returns expense totals grouped by category for a full year.
func (s *ReportService) GetYearlyCategoryTotals(ctx context.Context, year int) ([]domain.CategorySummary, error) {
	return s.reports.YearlyCategoryTotals(ctx, year)
}

// GetAllTimeCategoryTotals returns expense totals grouped by category across all time.
func (s *ReportService) GetAllTimeCategoryTotals(ctx context.Context) ([]domain.CategorySummary, error) {
	return s.reports.AllTimeCategoryTotals(ctx)
}
