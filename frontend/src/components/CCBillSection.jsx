...
export default function CCBillSection({ billId }) {
      ...
      >
        ...
        {bill && !loading && (
          <span style={{ marginLeft: 'auto', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--text-muted)', textTransform: 'none', letterSpacing: 0 }}>
            {fmtAmount(bill.total_amount, bill.currency)}
          </span>
        )}
      </button>
          ...
          {!loading && bill && (
            <>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 12, fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--mono)' }}>
                <span>Período {fmtDate(bill.period_from)} – {fmtDate(bill.period_to)}</span>
                {bill.due_date && <span>Vence {fmtDate(bill.due_date)}</span>}
              </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text-muted)' }}>
                      {fmtAmount(purchases.reduce((s, it) => s + it.amount, 0), bill.currency)}
                    </span>
                  </div>
                  {purchases.map(it => (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    ...
                    {commissions.length > 1 && (
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--red)' }}>
                        {fmtAmount(commissions.reduce((s, it) => s + it.amount, 0), bill.currency)}
                      </span>
                    )}
                  </div>
                  {commissions.map(it => (
